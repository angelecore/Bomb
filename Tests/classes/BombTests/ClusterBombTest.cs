...
{
    public class ClusterBombTest
    {

        private readonly ITestOutputHelper output;

        public ClusterBombTest(ITestOutputHelper output)
        {
            this.output = output;
        }
        [Fact]
        public void testClusterBombExplosionPositionsReturn()
        {
            Player player = new Player("temp id", "name", new Vector2f(3, 3));
            player.BombType = BombType.Cluster;
